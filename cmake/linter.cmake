...
# --------------------------------------------------------------------------------------------------
# Configures source code formatting tools Apply auto-formatting by calling add_clang_format() on
# target
# --------------------------------------------------------------------------------------------------
...
function(add_clang_format _targetname)
  if(CLANG_FORMAT_BIN AND ENABLE_FORMATTER)
    if(NOT TARGET ${_targetname})
      message(
        FATAL_ERROR "add_clangformat should only be called on targets (got \"${_targetname}\")")
    endif()
...
if(ENABLE_LINTER)
  ...
  if(LINTER_BIN)
    ...
    set(LINTER_INVOKE_COMMAND
        ${CMAKE_SOURCE_DIR}/cmake/clang-tidy-cache.py ${LINTER_BIN} -p ${CMAKE_BINARY_DIR}
